...
class AccuracyEvaluator:
    ...
    def load_data(self):
        # Load question data
        self.df = pd.read_csv(self.question_file, header=None)
        self.df = self.df.dropna(subset=[1])
        self.df = self.df.reset_index(drop=True)

        # Load dataset
        self.df2 = pd.read_csv(self.dataset)
        self.df2 = self.df2[['Title', 'Description']]
    ...
    def encode_data(self):
        ...
        encoded_data = self.model.encode(self.df2['Description'].tolist())
        ...
        if self.similarity == "cosine":
            # Normalize using L2 for cosine similarity
    ...
    def calculate_accuracy(self, total_queries, top_k):
        ...
        for i in range(total_queries):
            ...
            for result in results:
                if description in result.get('Description'):
                    correct_count += 1
                    break
    ...
    def calculate_precision_recall(self, total_queries, top_k, relevance_threshold=0.5):
        precision_at_k = []
        recall_at_k = []
        ...
        for i in range(total_queries):
            ...
            results = self.search_stack(query, top_k=top_k)
            query_relevant_docs = [idx for idx, result in enumerate(results) if result.get('Description') >= relevance_threshold]

            num_relevant_docs = len(query_relevant_docs)
            num_retrieved_docs = min(top_k, len(results))

            num_correct = len([doc for doc in results[:num_retrieved_docs] if doc['corpus_id'] in query_relevant_docs])

            precision_at_k.append(num_correct / num_retrieved_docs)
            recall_at_k.append(num_correct / num_relevant_docs)
...
if __name__ == '__main__':
    ...
    #precision_at_k, recall_at_k = evaluator.calculate_precision_recall(total_queries, top_k)

    # Write accuracy, precision, and recall to the output file
# Write accuracy, precision, and recall to the output file
